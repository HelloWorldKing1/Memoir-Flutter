...
| 富文本编辑器 | appflowy_editor 6.2+ | WYSIWYG 编辑（块组件/快捷键/斜杠命令/工具栏） |
| Markdown 渲染 | flutter_markdown 0.7+ | 详情页 Markdown 渲染 |
...
### 日记编辑器（AppFlowyEditor 集成）

**架构决策**：编辑器使用 **appflowy_editor 6.2** 提供 WYSIWYG 体验，内容以 **Markdown 格式存储**（与 PocketBase / Hive 完全兼容，无需 Schema 变更）。

**核心数据流**：
```dart
// 加载：Markdown → Document 节点树
final document = markdownToDocument(diary.content);
_editorState = EditorState(document: document);

// 保存：Document 节点树 → Markdown
final content = documentToMarkdown(_editorState.document);
// → diary.content → PocketBase / Hive
```

**编辑器配置** (`diary_editor_page.dart`):
- `blockComponentBuilders: standardBlockComponentBuilderMap` — 14 种块组件（段落/标题/列表/引用/分割线/表格等）
- `characterShortcutEvents: [...standardCharacterShortcutEvents, ...markdownSyntaxShortcutEvents]` — Markdown 语法自动转换（`#` → 标题, `**` → 加粗, `-` → 列表等）
- `commandShortcutEvents: [...standardCommandShortcutEvents, ...toggleMarkdownCommands]` — 键盘快捷键（Ctrl+B/I/S 等）
- `editorScrollController: EditorScrollController(editorState:)` — 滚动控制（必须传入 editorState）

**桌面端固定工具栏**：自定义 `_ToolIconButton` 按钮组，通过 `_editorState.selectionNotifier` 监听选区变化以更新高亮状态。按钮直接调用 `EditorState.toggleAttribute()` / `formatNode()` API，不依赖内置 `ToolbarItem`（其 builder 内部有空断言，要求选区非空）。

**移动端**：不显示固定工具栏，利用 AppFlowyEditor 内置的浮动选区工具栏 + 斜杠命令菜单（`/`）。

**主题适配**：`EditorStyle.desktop()` + `TextStyleConfiguration` 映射 shadcn_ui 的 `colorScheme`（光标色、选区色、7 种文本样式、行高）。

**已知问题**：
- Flutter 3.44 新增了 `TextInputClient.onFocusReceived()` 抽象方法，appflowy_editor 6.2 未实现。需手动修补 pub cache 中的两个文件：
  - `delta_input_service.dart` → `DeltaTextInputService.onFocusReceived() => false`
  - `non_delta_input_service.dart` → `NonDeltaTextInputService.onFocusReceived() => false`
- 运行 `flutter pub get` 后补丁会丢失，需重新应用（详见补丁脚本）。
- 等待 appflowy_editor 发布新版本后可移除补丁。
